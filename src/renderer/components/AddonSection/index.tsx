...
export const AddonSection = (): JSX.Element => {
    ...
    const publisherData = useAppSelector(state => state.configuration.publishers.find((pub:any) => pub.name === publisherName) ? state.configuration.publishers.find((pub:any) => pub.name === publisherName) : state.configuration.publishers[0]);
    ...
    useEffect(() => {
        const hiddenAddon = publisherData.addons.find((addon:any) => addon.key === selectedAddon.hidesAddon);
    ...
    useEffect(() => {
        const firstAvailableAddon = publisherData.addons.find((addon:any) => addon.enabled);
        ...
        const addonToSelect = publisherData.addons.find((addon:any) => addon.key === lastSeenAddonKey) || publisherData.addons.find((addon:any) => addon.key === firstAvailableAddon.key);
                ...
                <div className="h-full flex flex-col divide-y divide-gray-700">
                    <AddonBar>
                        <div className="flex flex-col gap-y-4">
                            {publisherData.addons.filter((it:any) => !it.category).map((addon:any) => (
                                <AddonBarItem
                        ...
                        <div className="h-full flex flex-col gap-y-4">
                            {publisherData.defs?.filter((it:any) => it.kind === 'addonCategory').map((category: AddonCategoryDefinition) => {
                                const categoryAddons = publisherData.addons.filter((it:any) => it.category?.substring(1) === category.key);
                                ...
                                return (
                                    <div className={classes}>
                                        ...
                                        <div className="flex flex-col gap-y-4">
                                            {publisherData.addons.filter((it:any) => it.category?.substring(1) === category.key).map((addon:any) => (
                                                <AddonBarItem
                                                    selected={selectedAddon.key === addon.key && addon.enabled}
                                                    enabled={addon.enabled || !!addon.hidesAddon}
                                                    addon={addon}
            ...
            >
                <div className="flex flex-row h-full relative">
                    <div className="w-full">
                        <Route path={`/addon-section/FlyWithJet Simulations/configuration/fbw-local-api-config`}>
                            <LocalApiConfigEditUI />
                        </Route>

                        <Route exact path={`/addon-section/${publisherName}`}>
                            {publisherData.addons.every((addon:any) => !addon.enabled) ?
